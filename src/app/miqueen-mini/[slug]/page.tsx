// ============================================
// UMÍSTĚNÍ: src/app/miqueen-mini/[slug]/page.tsx
// ============================================
...
export async function generateMetadata({
  ...
  if (!wine) {
    return { title: "Produkt nenalezen | MiQueen Mini" };
  }

  const isSet = wine.category === "set";
  const productType = isSet ? "Degustační set" : "Mini víno";
  const volume = isSet ? `4× ${wine.volume}ml` : `${wine.volume}ml`;

  return {
    title: `${wine.name} ${volume} | MiQueen Mini – ${wine.price} Kč`,
    description: `${wine.description} ${productType} v balení ${volume} za ${wine.price} Kč. ${wine.variety}, ${wine.vintage}. Moravské víno z vinařství MiQueen.`,
...
export default async function MiniWineProductPage({ params }: PageProps) {
  ...
  // DŮLEŽITÉ ZMĚNY:
  //
  // 1. Product má `sku` (povinné pro GSC "Záznamy obchodníka")
  //    → Generuje se z wine.id jako "MQ-MINI-{id}"
  //
  // 2. Offer má `url` (povinné — odkaz na stránku kde jde produkt koupit)
  //
  // 3. ODSTRANĚNO falešné aggregateRating
  //    → Originál počítal: ratingCount = Math.floor(wine.rating * 20)
  //    → 4.5 hvězd → 90 recenzí — to je vymyšlené, Google to penalizuje
  //    → Pokud máte reálné recenze, přidejte je z databáze
  //
  // 4. Sloučeno do @graph místo 2 separátních <script> bloků
  //
  // 5. availability jako plné URL (https://schema.org/InStock)
  // ────────────────────────────────────────
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 mt-20">
          ...
          <div className="grid md:grid-cols-2 gap-8 md:gap-12 lg:gap-16">
            ...
            <div>
              {/* Header */}
              <div className="mb-6">
                <p className="text-[#ab8754] font-medium mb-2">
                  {productType}
                </p>
              ...
              <div className="bg-gradient-to-r from-[#ab875410] to-transparent p-6 rounded-xl mb-6">
                ...
                <p className="text-gray-600 mt-2">
                  {isSet
                    ? `Degustační set 4× ${wine.volume}ml`
              ...
              {wine.notes && (
                <div className="bg-[#ab875410] p-4 rounded-xl mb-8">
                  <p className="text-[#ab8754] font-semibold mb-2">
                    Poznámka vinaře
                  </p>
          ...
          <div className="mt-16 md:mt-20">
            <div className="text-center mb-8 md:mb-12">
              ...
              <p className="text-gray-600">
                Objevte další vína z naší mini kolekce
              </p>
            </div>
                    ...
                    >
                      ...
                      <div className="p-3 md:p-4">
                        ...
                        <div className="mb-3">
                          ...
                          >
                            {isRelatedSet
                              ? `Set 4× ${relatedWine.volume}ml`
                              : `Mini ${relatedWine.volume}ml`}
                          </span>
                        </div>
